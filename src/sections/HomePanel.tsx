...
export function HomePanel({ accent, dark }: HomePanelProps) {
  const tags = ["Inference", "Evaluation", "Problem Framing", "Applied AI Systems"];

  return (
    <div className="h-full w-full overflow-y-auto px-6 pb-28 pt-24 md:px-16 md:pb-24 md:pt-20">
      <div className="relative mx-auto grid min-h-full w-full max-w-6xl items-center gap-12 py-8 md:grid-cols-[0.95fr_0.9fr] xl:max-w-7xl xl:gap-16">
        {/* Home anchor mark: above background, below main content */}
        <div
          className="
            pointer-events-none absolute z-0 hidden h-32 w-32 p-4 text-[#fbfaf7]
            shadow-[18px_18px_0_rgba(0,87,255,0.70)]
            md:left-0 md:top-[calc(50%+11rem)] md:block
            xl:-left-[5vw] xl:top-auto xl:-bottom-10
            2xl:-left-[12vw]
          "
        ...
        >
          ...
          <div className="mt-8 text-3xl font-light leading-none">
            2026
          </div>
          <div className="mt-2 text-[10px] uppercase tracking-[0.24em] text-white/65">
            AI / ML Archive
          </div>
        </div>
        ...
        <div className="relative z-10">
          ...
          <p className="mt-8 max-w-xl text-lg leading-8 text-neutral-700">
            I build evaluation-driven machine learning systems across inference,
            agent reliability, MLOps, and applied AI infrastructure for
            high-stakes domains.
          </p>
          ...
          <div className="mt-10 flex flex-wrap gap-3 text-[10px] uppercase tracking-[0.24em]">
            {tags.map((tag) => (
              <span
                key={tag}
                className="border border-neutral-950 bg-[#fbfaf7] px-3 py-2 text-neutral-700"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="relative z-10">
          <StampFrame accent={accent}>
            ...
            <p className="mt-8 max-w-md text-sm leading-7 text-neutral-700 md:mt-9">
              Reliability begins where model behavior, system constraints, and
              evaluation standards meet.
            </p>
